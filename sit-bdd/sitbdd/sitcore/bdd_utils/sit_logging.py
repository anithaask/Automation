...
LOG_FILE = os.path.join(CONFIG['staging'], LOG_DIR, LOG_FILE_NAME)
...
def setup_logger():
    ...
    if CONFIG["trace_logging"] is True:
...
class SITCoreLogger(logging.getLoggerClass()):
    """
...
class SITCoreLoggerFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if record.levelno == LOG_LEVEL_TRACE:
            ...
            self._fmt = SITCoreLoggerFormatter.trace_fmt
            self._style._fmt = SITCoreLoggerFormatter.trace_fmt
...
log_formatter = SITCoreLoggerFormatter()
...
def get_sit_logger():
    ...
    if cls is not SITCoreLogger:
        logging.setLoggerClass(SITCoreLogger)

    return logging.getLogger("SITLogger")
